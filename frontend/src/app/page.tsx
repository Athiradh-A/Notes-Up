...
export default function Home() {
  ...
  const downloadNotes = (notes: ActiveNotes) => {
    ...
    noteList.forEach((note, index) => {
      ...
      note.sections?.forEach((section) => {
        ...
        section.equations?.forEach((equation) => {
          lines.push("~~~text", cleanGeneratedText(equation), "~~~", "");
        });
      });
    ...
    anchor.href = url;
    anchor.download = "notes-up-study-guide.md";
              ...
              >
                <div className="p-6 md:px-8 border-b border-white/10 flex justify-between items-center bg-white/[0.025] backdrop-blur-xl">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => downloadNotes(activeNotes)} className="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold flex items-center gap-2 transition-all">
                      <Download size={16} />
                      Download .md
                    </button>
